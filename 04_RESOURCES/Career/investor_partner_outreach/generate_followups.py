#!/usr/bin/env python3
"""
Phase 1 Follow-up Draft Generator
Reads outreach_crm.json, generates follow-up drafts for entries that were sent but haven't responded.
"""
...
def get_original_subject(company):
    """Get the original email subject from the draft file."""
...
for entry in entries:
    ...
    # Only generate follow-ups for sent emails with no response
    if sent_status == "sent" and response_status == "pending":
        # For now, assume all were sent on Aug 9, 2026
        ...
        safe_company = company.replace(" ", "_").replace(".", "").replace("&", "and").replace(",", "")
        safe_contact = f"{entry.get('contact_name', 'unknown').replace(' ', '_')}_{entry.get('contact_position', 'contact').replace(' ', '_')}"
        
        if days_since >= 3 and days_since < 10 and not entry.get("followup_1_sent"):
            # Follow-up 1
            draft = FOLLOWUP_1_TEMPLATE.format(
            ...
            filename = f"FOLLOWUP1_{sent_date}_{safe_company}_{safe_contact}.md"
            filepath = followup_dir / filename
            with open(filepath, "w") as f:
                f.write(draft)
            followup_1_count += 1
            print(f"Generated Follow-up 1: {filename}")
            
        elif days_since >= 10 and days_since < 17 and entry.get("followup_1_sent") and not entry.get("followup_2_sent"):
            # Follow-up 2
            draft = FOLLOWUP_2_TEMPLATE.format(
                contact_email=contact_email,
                contact_name=contact_name.split()[0] if contact_name else "Team",
                original_subject=original_subject,
                company=company,
            ...
            filename = f"FOLLOWUP2_{sent_date}_{safe_company}_{safe_contact}.md"
            filepath = followup_dir / filename
            with open(filepath, "w") as f:
                f.write(draft)
            followup_2_count += 1
            print(f"Generated Follow-up 2: {filename}")
            
        elif days_since >= 17 and entry.get("followup_2_sent") and not entry.get("breakup_sent"):
            # Breakup email
            draft = BREAKUP_TEMPLATE.format(
                contact_email=contact_email,
                contact_name=contact_name.split()[0] if contact_name else "Team",
                company=company
            )
            filename = f"BREAKUP_{sent_date}_{safe_company}_{safe_contact}.md"
            filepath = followup_dir / filename
            with open(filepath, "w") as f:
                f.write(draft)
            breakup_count += 1
            print(f"Generated Breakup: {filename}")
