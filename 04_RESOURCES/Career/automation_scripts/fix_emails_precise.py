...
# Build a map: (company_name_normalized, first_name_normalized, last_name_normalized) -> email
# We'll also keep a map by company only for fallback to first contact (if we must)
company_to_first_email = {}
contact_map = {}
for company_name, info in data['contacts'].items():
    # Normalize company name for matching
    company_norm = re.sub(r'[^\w\s]', '', company_name).lower().strip()
    # Store first email for company (if any)
    if info['top_contacts']:
        first_email = info['top_contacts'][0].get('value')
        if first_email:
            company_to_first_email[company_norm] = first_email
...
for draft_file in drafts_dir.glob('*.md'):
    ...
    # If we found an email, update the draft
    if email:
        ...
        # print(f"No match for {draft_file.name} (company: {company_name_from_file}, first: {first}, last: {last})")
